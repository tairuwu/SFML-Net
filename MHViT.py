...
class MHSA(torch.nn.Module):
    """Multi-Head Self-Attention (针对局部通道)"""

    def __init__(self, dim, qk_dim, pdim, num_heads=2):
        super().__init__()
        assert qk_dim % num_heads == 0, "qk_dim must be divisible by num_heads"
        assert pdim % num_heads == 0, "pdim must be divisible by num_heads"
    ...
    def forward(self, x):
        ...
        # 3. (B, heads, head_dim, N)
        N = H * W
        q = q.reshape(B, self.num_heads, self.qk_dim // self.num_heads, N)
        k = k.reshape(B, self.num_heads, self.qk_dim // self.num_heads, N)
        v = v.reshape(B, self.num_heads, self.pdim // self.num_heads, N)
...
class BasicBlock(torch.nn.Module):
    def __init__(self, dim, qk_dim, pdim, type, num_heads=2): # 增加 num_heads 参数
        super().__init__()
        if type == "s":
            self.conv = Residual(Conv2d_BN(dim, dim, 3, 1, 1, groups=dim, bn_weight_init=0))
            # 使用 MHSA 替换 SHSA
            self.mixer = Residual(MHSA(dim, qk_dim, pdim, num_heads=num_heads))
...
class SHViT(torch.nn.Module):
    def __init__(self,
        ...
        # Patch embedding
        self.patch_embed = torch.nn.Sequential(Conv2d_BN(in_chans, embed_dim[0] // 8, 3, 2, 1),   #112*112
                                               torch.nn.ReLU(),
                                               Conv2d_BN(embed_dim[0] // 8, embed_dim[0] // 4, 3, 2, 1), #56*56
                                               torch.nn.ReLU(),
    ...
    def forward(self, x):
        ...
        x = splited[1]  # (8,3,224,224)
        x = self.patch_embed(x)
        x = self.blocks1(x)  #([8, 128, 14, 14])
        # print("1",x.size())   #28
        x = self.blocks2(x)    #14
        # print("2",x.size())  #([8, 256, 7, 7])
        x = self.blocks3(x)    #7
        # print("3",x.size())  #([8, 512, 7, 7])
